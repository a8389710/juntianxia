<template>
  <div class="cart">
    ...
    <van-list class="food-list" @load="onLoad">
      <div
        v-if="cartList == 0"
        style="color: #323233; text-align: center; font-size: 14px; margin-top: 20px;"
      >暂时没有菜品哦</div>
      ...
      >
        ...
        <div slot="desc">
          <van-rate v-model="star_val" allow-half void-icon="star" void-color="#eee" size="14px" />
          <span class="star-num">{{item.score}}分</span>
          <span class="sold-num">已售{{item.num}}</span>
        </div>
        <div slot="desc" class="desc">{{item.goods_content}}</div>
    ...
    <van-submit-bar :price="getTotal.totalPrice*100" button-text="立即预定" @submit="toPay">
      ...
      <!-- <van-checkbox @click="selectCart(isSelectAll)" v-model="isSelectAll" >全选</van-checkbox> -->
      <span slot="tip">支付即表示您已同意付费预定规则说明</span>
    </van-submit-bar>
    <!--    </van-checkbox-group>-->
  </div>
</template>
        ...
        this.Api.post("api/dining_car/addreserve", req)
          .then(res => {
            ...
            if (res.code == 0) {
              ...
              console.log("添加订单失败", this.orderId, res, req);
            }
          })
          .catch(err => {
            console.log(err);
