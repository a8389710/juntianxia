      ...
      <div :class="(key%2 ==0)?left:right" v-for="(item,key) in potList" :key="item.id">
        ...
        <div class="detail">
          <div>
            ...
            <div class="gd">
              <p class="pot-choice"><span ref=ld v-for="(ld,index) in item.ld" :key="ld" :class="activeClass[index]?'active':''"
                                          @click="ldChecked(key,index)">{{ld}}</span></p>
              <p class="soup-choice"><span v-for="(tl,index) in item.tl"  :key="tl.id" :class="activeClass[index]?'active':''"
                                           @click="tlChecked(key,index)">{{tl}}</span></p>
            </div>
            <p class="price">
          ...
          >
            ...
            >
              ...
              <div slot="desc">
                ...
                <span class="sold-num">已售{{food.num}}</span>
              ...
              <div slot="footer" class="stepper">
                <van-stepper v-model="food.goods_num" button-size="26px"  @plus="num1(food.id,food.goods_num)" @minus="num2(food.id,food.goods_num)"/>
    ...
    methods: {
      ...
      addPot(key) {
        ...
        if (this.reserve_id) {
          ...
          this.Api.post('api/reserve_pot/add', req)
            .then(data => {
              ...
              if (data.code == 0) {
                ...
                this.pot_price = parseInt(data.data.pot_price)
